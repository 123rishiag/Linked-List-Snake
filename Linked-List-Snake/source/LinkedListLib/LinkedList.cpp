...
namespace LinkedListLib
{
	...
	int LinkedList::findMiddleNode() const
	{
		Node* slow = head_node;
		Node* fast = head_node;
	...
	Node* LinkedList::findNodeAtIndex(int index) const
	{
		int current_index = 0;
		Node* cur_node = head_node;
	...
	bool LinkedList::processNodeCollision() const
	{
	...
	sf::Vector2i LinkedList::getNewNodePosition(Node* reference_node, Operation operation) const
	{
		switch (operation)
		{
		case Operation::HEAD:
	...
	Direction LinkedList::getReverseDirection(Direction reference_direction) const
	{
	...
	int LinkedList::getLinkedListSize() const
	{
		return linked_list_size;
	}

	std::vector<sf::Vector2i> LinkedList::getNodesPositionList() const
	{
